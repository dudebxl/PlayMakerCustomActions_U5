...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Store all active GameObjects from inside a 2D collider with a specific tag and/or layer. Will filter first by tag then by layer. Tags and/or layers must be declared in the tag/layer manager before using them")]
	[HelpUrl("http://hutonggames.com/playmakerforum/index.php?topic=11754.0")]
	public class ArrayListFindGameObjectsInsideCollider2D : ArrayListActions
	{
		...
		public void FindGOByTag()
		{
			...
			if (layerFilterOn.Value == false)
			{
				...
				for(int i = 0; i<objtag.Length;i++){
					...
					Sprite tempRen = objtag[i].GetComponent<Sprite>();
