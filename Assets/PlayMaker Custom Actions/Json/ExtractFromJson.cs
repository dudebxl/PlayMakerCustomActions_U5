...
// Keywords: Extract string value from Json
...
{
	[ActionCategory("Json")]
	[Tooltip("Extract value from simple Json to string")]
	[HelpUrl("http://hutonggames.com/playmakerforum/index.php?topic=11991.0")]
	public class ExtractFromJson : HashTableActions
	{
		
		[ActionSection("Set up")]
		[Tooltip("Item to extract")]
		public FsmString extract;
		...
		public void ExtractStringValue()
		{
			...
			if (extract.Value == null || extract.IsNone ) {
				Fsm.Event(failureEvent);	
				Debug.LogWarning("<b>[HashTableAddFromJson]</b><color=#FF9900ff> Extract Value is null - Please review!</color>", this.Owner);
				return;
			...
			if (result.Value == null || result.IsNone ) {
				Fsm.Event(failureEvent);	
				Debug.LogWarning("<b>[HashTableAddFromJson]</b><color=#FF9900ff> Result is null - Please review!</color>", this.Owner);
				return;
		...
		public void Convert()
		{
			...
			var dict = JsonSimple.Deserialize( jsonSource.Value ) as Dictionary <string, object>;


			result.Value = System.Convert.ToString(dict[extract.Value]);
